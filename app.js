...
// Memos API 配置
const MEMOS_URL = process.env.MEMOS_URL; // 例如 http://ip:5230/api/v1/memos
const MEMOS_WEB_URL = process.env.MEMOS_WEB_URL || 'http://your-memos-ip:5230';

// 默认可见性/标签
const DEFAULT_VISIBILITY = (process.env.MEMOS_VISIBILITY || 'PRIVATE').toUpperCase();
const DEFAULT_TAG = process.env.MEMOS_DEFAULT_TAG || '#企业微信机器人';

// 无菜单模式（环境变量控制，默认 false）
const NO_MENU = String(process.env.NO_MENU || 'false').toLowerCase() === 'true';
...
const userContext = new Map(); // key: userId, value: { mode, page, totalPages, keyword, pageSize, hasMore }
...
function assertEnv() {
  ...
  if (!ENCODING_AES_KEY) missing.push('WECOM_ENCODING_AES_KEY');
  if (!MEMOS_URL) missing.push('MEMOS_URL');

  if (missing.length) {
    log('error', `缺少必要环境变量：${missing.join(', ')}`);
    log('error', '请检查 docker-compose.yml 或 docker run 的 -e 配置后再启动。');
    process.exit(1);
  }

  if (!['PRIVATE', 'PROTECTED', 'PUBLIC'].includes(DEFAULT_VISIBILITY)) {
    log('warn', `MEMOS_VISIBILITY=${DEFAULT_VISIBILITY} 非法，已回退为 PRIVATE`);
  }

  if (String(ENCODING_AES_KEY).length != 43) {
    log('warn', `WECOM_ENCODING_AES_KEY 长度应为 43，当前为 ${String(ENCODING_AES_KEY).length}，可能导致回调验签/解密失败。`);
  }
}
...
function formatCnTime(isoString) {
  // 统一使用 Asia/Shanghai，避免手动 +8 与服务器时区叠加导致时间偏差
...
// ---------- 企业微信签名校验（SHA1） ----------
// 官方规则：dev_msg_signature = sha1(sort(token, timestamp, nonce, msg_encrypt))
// 其中 msg_encrypt 对 GET 为 echostr，对 POST 为 XML 中 Encrypt 节点内容
function calcWecomSignature(token, timestamp, nonce, encrypt) {
  const arr = [token, timestamp, nonce, encrypt].map(x => String(x || ''));
  arr.sort();
  const joined = arr.join('');
  return crypto.createHash('sha1').update(joined).digest('hex');
}

function verifyWecomSignature(msgSignature, timestamp, nonce, encrypt) {
  if (!msgSignature || !timestamp || !nonce || !encrypt) return false;
  const expected = calcWecomSignature(TOKEN, timestamp, nonce, encrypt);
  // 固定时间比较
  return crypto.timingSafeEqual(Buffer.from(expected), Buffer.from(String(msgSignature)));
}
...
function getUserVisibility(userId) {
  const user = userData.get(userId);
  const vis = user ? user.visibility : DEFAULT_VISIBILITY;
  return ['PRIVATE', 'PROTECTED', 'PUBLIC'].includes(vis) ? vis : 'PRIVATE';
}
...
async function sendMessage(touser, content) {
  ...
  log('debug', `准备发送消息给 ${touser}: ${String(content).substring(0, 50)}...`);
...
async function updateWecomMenu() {
  ...
  if (globalMeta.menuEnabled) {
    ...
    payload = {
      button: [{ type: 'click', name: '开启菜单', key: 'toggle_menu' }]
    };
  }
...
// ---------- 核心查询函数（list/search/search_all/today/week/filter） ----------
async function _doList(userId, page) {
  ...
  const data = await handleMemosRequest(userId, token => callMemosApi('GET', '', token, null, params));
  ...
  pageMemos.forEach((m, idx) => {
    const date = formatCnTime(m.createTime).slice(5, 16);
    const circled = numToCircled(start + idx + 1);
    const snippet = String(m.content || '');
    reply += `\n${circled} ${date}\n${snippet.substring(0, 30)}${snippet.length > 30 ? '…' : ''}`;
  });
...
async function _doSearch(userId, keyword, page) {
  ...
  const data = await handleMemosRequest(userId, token => callMemosApi('GET', '', token, null, { filter, pageSize: fetchSize }));
  const memos = data.memos || [];
...
async function _doSearchAll(userId, keyword, page) {
  const pageSize = 3;
  const filter = `content.contains('${keyword}')`;
  const data = await handleMemosRequest(userId, token => callMemosApi('GET', '', token, null, { filter, pageSize: 100 }));
  const memos = data.memos || [];
  ...
  pageMemos.forEach((m, idx) => {
    const date = formatCnTime(m.createTime);
    const circled = numToCircled(start + idx + 1);
    reply += `**${circled} ${date}**\n${m.content}\n\n`;
  });
...
async function cmdList(userId, args) {
  const page = args.length ? parseInt(args[0], 10) : 1;
  ...
  if (ctx && (ctx.mode === 'search' || ctx.mode === 'search-all')) {
    ...
    return await _doSearchAll(userId, ctx.keyword, page);
  }
...
async function cmdToday(userId) {
  // 以 Asia/Shanghai 的“当天”范围过滤（使用本地日期计算）
  ...
  try {
    const data = await handleMemosRequest(userId, token => callMemosApi('GET', '', token, null, { filter, pageSize: 50 }));
    const memos = data.memos || [];
    if (memos.length === 0) return '📭 今天还没有备忘录。';

    userLastResults.set(userId, memos);
    userCurrentIndex.delete(userId);
    userContext.set(userId, { mode: 'today', page: 1, totalPages: 1, pageSize: memos.length, hasMore: false });
    ...
    memos.forEach((m, idx) => {
      ...
      const snippet = String(m.content || '');
      reply += `\n${circled} ${date}\n${snippet.substring(0, 50)}${snippet.length > 50 ? '…' : ''}`;
    });
    return reply;
  } catch (err) {
...
async function cmdWeek(userId) {
  ...
  try {
    ...
    userLastResults.set(userId, memos);
    userCurrentIndex.delete(userId);
    userContext.set(userId, { mode: 'week', page: 1, totalPages: 1, pageSize: memos.length, hasMore: false });

    let reply = `【本周模式】\n📋 **本周备忘录 (${memos.length}条)**\n`;
    memos.forEach((m, idx) => {
      const date = formatCnTime(m.createTime).slice(5, 10);
      const circled = numToCircled(idx + 1);
      const snippet = String(m.content || '');
      reply += `\n${circled} [${date}] ${snippet.substring(0, 50)}${snippet.length > 50 ? '…' : ''}`;
    });
    return reply;
  } catch (err) {
...
async function cmdFilter(userId, args) {
  ...
  userLastResults.set(userId, memos);
  userCurrentIndex.delete(userId);
  userContext.set(userId, { mode: 'filter', page: 1, totalPages: 1, pageSize: memos.length, hasMore: false });
...
async function cmdView(userId, args) {
  ...
  const memoId = String(memo.name || '').split('/')[1] || 'unknown';
  const date = formatCnTime(memo.createTime);
...
async function cmdId(userId) {
  ...
  return String(memo.name || '').split('/')[1] || 'unknown';
}
...
async function cmdUp(userId) {
  ...
  const ctx = userContext.get(userId);
  if (!ctx) return '❌ 没有正在进行的查询，请先执行 /list、/search 或 /search_all。';
  const newPage = ctx.page - 1;
  if (newPage < 1) return '❌ 已经是第一页了。';

  if (ctx.mode === 'list') return await _doList(userId, newPage);
  if (ctx.mode === 'search') return await _doSearch(userId, ctx.keyword, newPage);
  if (ctx.mode === 'search-all') return await _doSearchAll(userId, ctx.keyword, newPage);
  return '❌ 当前模式不支持翻页。';
}
...
async function cmdDown(userId) {
  ...
  if (ctx.mode === 'list') {
    ...
    if (result.includes('📭 第 ') && result.includes('页无内容')) return '❌ 已经是最后一页了。';
    return result;
  }
  if (ctx.mode === 'search') {
...
async function cmdGet(userId, args) {
  ...
  const date = formatCnTime(data.createTime);
  return `📄 **备忘录 ${memoId}**\n创建时间：${date}\n可见性：${data.visibility}\n置顶：${data.pinned ? '是' : '否'}\n内容：\n${data.content}`;
}
...
async function cmdTags(userId) {
  ...
  memos.forEach(m => {
    const tags = String(m.content || '').match(/#[a-zA-Z0-9_\u4e00-\u9fa5]+/g) || [];
...
async function cmdSetVisibility(userId, args) {
  ...
  if (!['PRIVATE', 'PROTECTED', 'PUBLIC'].includes(vis)) {
    if (visibilityMap[input]) vis = visibilityMap[input];
    else return '❌ 无效的可见性，请选择：' + Object.values(visibilityDisplay).join('、');
  }
...
// ---------- 回调接口 ----------
// URL 验证：GET /callback?msg_signature=...&timestamp=...&nonce=...&echostr=...
app.get('/callback', async (req, res) => {
  const { msg_signature, timestamp, nonce, echostr } = req.query;
  log('info', '收到 GET 验证请求');

  if (!verifyWecomSignature(msg_signature, timestamp, nonce, echostr)) {
    log('warn', 'URL 验证签名失败');
  ...
  try {
    ...
    log('error', 'URL 验证解密失败：', decrypted);
    return res.status(401).send('decrypt failed');
  } catch (err) {
    log('error', 'URL 验证异常：', err);
    return res.status(500).send('error');
  }
});

// 企业微信推送：POST /callback?msg_signature=...&timestamp=...&nonce=...
app.post('/callback', express.text({ type: 'text/xml' }), async (req, res) => {
  ...
  try {
    ...
    if (!verifyWecomSignature(msg_signature, timestamp, nonce, encryptNode)) {
      log('warn', 'POST 回调签名失败');
      return res.status(401).send('invalid signature');
    }

    const decrypted = wecomCrypto.decrypt(ENCODING_AES_KEY, encryptNode);
    if (!decrypted || !decrypted.message) throw new Error('解密失败');

    log('debug', '解密后的消息(前200)：', decrypted.message.substring(0, 200));
      ...
      try {
        ...
        if (content.startsWith('/')) {
          ...
          if (spaceMatch && spaceMatch[1].length > 1) {
            extraSpaceMsg = `\n(提示：已忽略 /${cmd} 与参数之间的 ${spaceMatch[1].length} 个空格)`;
          }
          ...
          const needTokenCmd = new Set([
            'mode','set_tag','set_visibility','get_visibility','exit','id','pure','up','down','bot_menu',
            'list','search','search_all','today','week','filter','view','get','update','delete','stats','tags','random','pin','visibility'
          ]);
          ...
          if ((effectiveCmd === 'help') || (effectiveCmd === '')) {
            ...
            if (needTokenCmd.has(effectiveCmd) && !userData.has(fromUser)) {
              replyContent = '❌ 请先使用 /start 设置你的 Memos 令牌。';
            } else {
              try {
                switch (effectiveCmd) {
                  case 'mode': replyContent = await cmdMode(fromUser); break;
                  case 'set_tag': replyContent = await cmdSetTag(fromUser, args); break;
                  case 'set_visibility': replyContent = await cmdSetVisibility(fromUser, args); break;
                  case 'get_visibility': replyContent = await cmdGetVisibility(fromUser); break;
                  case 'exit': replyContent = await cmdExit(fromUser); break;
                  case 'id': replyContent = await cmdId(fromUser); break;
                  case 'pure': replyContent = await cmdPure(fromUser); break;
                  case 'up': replyContent = await cmdUp(fromUser); break;
                  case 'down': replyContent = await cmdDown(fromUser); break;
                  case 'bot_menu': replyContent = await cmdBotMenu(fromUser); break;
                ...
                if (err.message === 'TOKEN_EXPIRED') replyContent = '❌ 令牌已过期，请重新 /start 设置。';
                else if (err.message === 'NO_TOKEN') replyContent = '❌ 请先使用 /start 设置你的 Memos 令牌。';
                else replyContent = `❌ 执行失败：${err.message}`;
              }
            }
          }
        } else {
          ...
          if (!userData.has(fromUser)) {
            ...
            try {
              ...
              log('error', '保存失败:', err);
              if (err.message === 'TOKEN_EXPIRED') replyContent = '❌ 令牌已过期，请重新 /start 设置。';
              else replyContent = '❌ 保存失败，请稍后重试。';
            }
          }
        }

        if (extraSpaceMsg && replyContent) replyContent += extraSpaceMsg;
        if (replyContent) await sendMessage(fromUser, replyContent);
      } catch (err) {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  if (NO_MENU) log('info', '无菜单模式已启用，动态菜单功能已禁用');
});
